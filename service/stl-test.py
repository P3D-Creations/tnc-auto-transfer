...
print("== TEST 3: off-centre attach warns + guarantees >= clearance ==")
rc, so, se = run([src, out, "--clearance", "1.5", "--attach", "-30,0,0"])
r = json.loads(so); bb = bbox(read_binary_stl(out))
check("exit 0", rc == 0, se)
...
rc, so, se = run([sp, spo, "--max-tris", "19500", "--clearance", "1.5", "--attach", "0,0,-25"])
check("exit 0", rc == 0, se)
